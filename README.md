...
- **Modular pipeline**: Parsing → Grouping → Scoring → Reporting, each step transparent and logged.  
- **Observability**: Full INFO/DEBUG logs, severity breakdowns, and average score metrics.  
- **AI Integration**: High‑severity incidents enriched with Gemini natural‑language explanations.  
- **Evaluator‑ready**: Sample input (`data/input/logs.csv`) and output (`output/report.md`) included for quick verification.  
...
- ✅ Parsing structured logs into event objects  
- ✅ Incident grouping by key fields (action, host, user, IP, time bucket)  
- ✅ Scoring incidents with simple heuristics (failed logins, admin targeting, port scans, suspicious IPs)  
- ✅ Reporting with summary stats, top incidents, and recommendations  
- ✅ Observability via logging and metrics  
- 🌟 **Gemini Integration**: High‑severity incidents include natural‑language explanations generated by Google Gemini, enriching numeric scores with AI‑powered narrative context  
...
> ⚠️ Keys are **never hard‑coded** in source files. This ensures security and portability.
```
...
- Output: `output/report.md` (Markdown triage report with Gemini explanations for high‑severity incidents)  
...
│   ├── parser.py       # Load events from CSV
│   ├── dedupe.py       # Group events into incidents
│   ├── score.py        # Apply scoring heuristics + Gemini integration
│   ├── report.py       # Generate and render report (includes Gemini explanations)
│   └── models.py       # Core data models
├── data/input/logs.csv # Sample input logs
├── output/report.md    # Sample output report
...
To help evaluators quickly verify Gemini integration, here’s an excerpt from `output/report.md`:
...
- 🌟 **Bonus Feature:** Gemini explanations enrich high‑severity incidents, demonstrating effective use of AI for contextual triage.  
